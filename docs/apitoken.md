...
| `src/lib/jwt.ts`                                        | Reads `JWT_SECRET` from `.env` and provides it for signing/verifying authentication cookies.                                                                                                       | `process.env.JWT_SECRET` (populated by Next.js at runtime)   |
